...
namespace SaifFuelMod
{
    public class FuelMod : Script
    {
        ...
        private enum HudStyle { FlatTank, SegmentedBar }
        private HudStyle _hudStyle = HudStyle.FlatTank;
        ...
        private NativeListItem<string> _hudStyleItem;
        ...
        private void BuildMenus()
        {
            ...
            _hudStyleItem = new NativeListItem<string>("HUD Style", "Flat Tank", "Segmented Bar");
            _hudStyleItem.SelectedIndex = _hudStyle == HudStyle.SegmentedBar ? 1 : 0;
            _hudStyleItem.ItemChanged += (s, e) => _hudStyle = _hudStyleItem.SelectedItem == "Segmented Bar" ? HudStyle.SegmentedBar : HudStyle.FlatTank;
            ...
            _settingsMenu.Add(_hudStyleItem);
        ...
        private void UpdateFuelAndOilDrain()
        {
            ...
            if (veh.IsEngineRunning)
            {
                ...
                float litresPerSecond = (5.0f / 100f) * modifier;
        ...
        private const float DELIVERY_FLIGHT_SECONDS = 12f;
        ...
        private void TryCallDelivery()
        {
            ...
            Function.Call(Hash.FREEZE_ENTITY_POSITION, _deliveryPlane, true); // we drive this one by hand, not the physics/AI
        ...
        private void UpdateDelivery()
        {
            ...
            switch (_deliveryPhase)
            {
                ...
                case DeliveryPhase.Leaving:
                    ...
                    _deliveryFlightT += Game.LastFrameTime / 6f;
        ...
        private void MovePlaneTo(Vector3 pos)
        {
            ...
            if (dir.Length() > 0.01f)
            {
                float heading = (float)(Math.Atan2(dir.X, dir.Y) * (180.0 / Math.PI)) * -1f + 90f;
        ...
        {
            if (!Game.Player.Character.IsInVehicle()) return; // no fuel HUD on foot
            ...
            if (_hudStyle == HudStyle.SegmentedBar)
                DrawSegmentedBarGauge(x, barTop, tankWidth, tankHeight, fuelPct);
            else
                DrawFlatTankGauge(x, barTop, tankWidth, tankHeight, fuelPct);
        ...
        private void DrawFlatTankGauge(float x, float barTop, float tankWidth, float tankHeight, float fuelPct)
        {
        ...
        // Vertical stack of segments (green -> yellow -> red toward empty), matching
        // the SincoTech-style gauge look from the reference image.
        private void DrawSegmentedBarGauge(float x, float barTop, float tankWidth, float tankHeight, float fuelPct)
        {
            const int SEGMENTS = 10;
            float segHeight = (tankHeight + 40f) / SEGMENTS - 3f;
            float colWidth = 34f * (_hudScale);

            new ContainerElement(new PointF(x - 8, barTop - 26), new SizeF(colWidth + 16, tankHeight + 40), Color.FromArgb(150, 0, 0, 0)).Draw();
            new TextElement("FUEL", new PointF(x, barTop - 22), 0.22f, Color.White, Font.ChaletLondon).Draw();
            ...
            for (int i = 0; i < SEGMENTS; i++)
            {
                ...
                Color segColor;
                if (!lit) segColor = Color.FromArgb(140, 40, 40, 40);
                else if (i < SEGMENTS * 0.3f) segColor = Color.Red;       // bottom segments (empty end)
                else if (i < SEGMENTS * 0.5f) segColor = Color.Orange;
                else segColor = Color.LimeGreen;

                float segY = barTop - 2 + i * (segHeight + 3f);
                new ContainerElement(new PointF(x, segY), new SizeF(colWidth, segHeight), segColor).Draw();
            }

            new TextElement($"{fuelPct * 100:F0}%", new PointF(x, barTop + tankHeight + 44), 0.24f, Color.White, Font.ChaletLondon).Draw();
        }
